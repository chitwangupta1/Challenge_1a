...
def save_headings_to_json(pdf_path):
    ...
    json_filename = f"{title}_headings.json"
    with open(json_filename, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    print(f"✅ Headings saved to '{json_filename}'")
...
def save_headings_to_json(pdf_path):
    ...
    json_filename = json_filename = f"output/{title}_headings.json"
    
    # path = "D:\College\Python Course\Python Projects\Pdf Crawler\Langchain_PC\Challenge_1a\output" + json_filename
    with open(json_filename, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)
...
PDF_DIR = "sample_pdfs/pdfs"
PDF_CHOICES = [
    "file01", "file02", "file03", "file04", "file05",
    "South of France - Cities",
    "South of France - Cuisine",
    "South of France - History",
    "South of France - Restaurants and Hotels",
    "South of France - Things to Do",
    "South of France - Tips and Tricks",
    "South of France - Traditions and Culture",
]

def main():
    print("📄 Choose a PDF file to extract headings:\n")
    for idx, name in enumerate(PDF_CHOICES, start=1):
        print(f"{idx}. {name}")

    choice = input("\nEnter the number corresponding to your file: ").strip()
    if not choice.isdigit() or not (1 <= int(choice) <= len(PDF_CHOICES)):
        print("❌ Invalid choice. Exiting.")
        return

    selected_name = PDF_CHOICES[int(choice) - 1]
    pdf_path = f"{PDF_DIR}/{selected_name}.pdf"

    if not Path(pdf_path).exists():
        print(f"❌ File not found: {pdf_path}")
        return

    save_headings_to_json(pdf_path)
    print(f"✅ Headings extracted and saved for: {selected_name}")
